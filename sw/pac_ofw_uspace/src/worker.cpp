...
void init_pac_ofw_ctx( pac_ofw_context_t& ctx, 
    ...
    LOG_INFO("Reset head\n");
    ctx.pci_vaddr[CSR_PAC_OFW_BUF_HEAD] = ctx.pac_ofw_buf_paddr + (BUF_SIZE_BYTE - 64);
    LOG_INFO("Reset done\n");
}
...
int dump_pac_ofw_buff(pac_ofw_context_t& ctx, const char* out_path) {
    ...
    for (uint32_t i = 0; i < CNT_TABLE_NUM_ENTRY; i++){ // loop through the 4MB counter buffer (2MB for 8bit counter), 1MB each time
        val = ctx.cnt_table[i];
        if (val > 0) {
            non_zero_cnt++;
            fprintf(out_fd, "%x %x\n", i, val);
...
void accu_pac_ofw(pac_ofw_context_t& ctx) {
    ...
    while (ctx.pci_vaddr[CSR_PAC_OFW_VALID_CNT] == ctx.prev_valid_cnt) {
        //LOG_INFO("Waiting for valid count to be set = %lu\n", pci_vaddr[CSR_PAC_OFW_VALID_CNT]);
        //sleep(SLEEP_SEC);
        //std::this_thread::sleep_for(std::chrono::milliseconds(1));
        std::this_thread::sleep_for(sleep_us);
        if (spin_cnt++ > 10000) {
            if (ctx.pci_vaddr[CSR_PAC_OFW_BUF_HEAD] == ctx.pac_ofw_buf_paddr) {
                ctx.pci_vaddr[CSR_PAC_OFW_BUF_HEAD] = ctx.pac_ofw_buf_paddr + BUF_SIZE_BYTE - 64;
            } else {
                ctx.pci_vaddr[CSR_PAC_OFW_BUF_HEAD] -= 64;
            }
            ctx.tmp_inc_cnt++;
            goto FAILED;
        }
    ...
    if (ctx.valid_cnt_diff >= (BUF_SIZE_BYTE / 64)) {
        ...
        ctx.valid_cnt_diff = (BUF_SIZE_BYTE / 64) - 1;
        LOG_ERROR("set new valid / diff = %lu / %lu \n", ctx.prev_valid_cnt, ctx.valid_cnt_diff);
        ctx.bad_diff_cnt++;
    }

    for (int i = 0; i < ctx.valid_cnt_diff * NUM_ADDR_PER_BLOCK; i++) {         // each valid count represents 512/32=16 addresses
        //LOG_INFO("0x%x", pac_ofw_buf_vaddr[i]);
        curr_dump_idx = ctx.pac_ofw_buf_vaddr[(ctx.prev_position + i) % BUF_SIZE_BLOCK] >> 6;
        if (curr_dump_idx < CNT_TABLE_NUM_ENTRY) {
            ctx.cnt_table[curr_dump_idx] += (1 << BITS_PER_SRAM_ENTRY);
        } else {
            LOG_ERROR("BAD index %u\n", curr_dump_idx);
        }
        //cout << "0x" << std::hex << curr_dump_idx << std::dec << " ";
    ...
    if (is_all_zero) {
        LOG_ERROR("the pac ofw did not touch this block\n");
        ctx.all_zero_cnt++;
    }

    // move head, unit = 32bit int (but should be CL aligned)
    // XXX, without this, diff = 63 will trigger head = tail
    if ((ctx.valid_cnt_diff - 1) % (ctx.valid_cnt_diff - 1) == 0) ctx.valid_cnt_diff -= 1;
    ctx.new_position = (ctx.prev_position + (ctx.valid_cnt_diff * NUM_ADDR_PER_BLOCK)) % BUF_SIZE_BLOCK;
    //LOG_INFO("pos new / old = %lu / %lu, diff: %lu\n", new_position, prev_position, valid_cnt_diff);
    ctx.old_head = ctx.pci_vaddr[CSR_PAC_OFW_BUF_HEAD];

    ctx.pci_vaddr[CSR_PAC_OFW_BUF_HEAD] = ctx.pac_ofw_buf_paddr + (ctx.new_position) * sizeof(uint32_t);
    ctx.prev_position = ctx.new_position;
    return;
...
int pac_ofw_func(char* dump_path, 
    ...
    while (!stop_flag) {
        ...
        // dump dmesg, if not using m5 for printing addr
        // legacy naming ...
        string name = file_path + "/offset_" + std::to_string(dump_cnt_local) + "/counter_val_0.txt";
        // dump & clear table
...
int threshold_policy_v2(
    ...
    freq_factor = pow(freq_factor, ratio_power);
    ...
    // finaliziing 
    double freq_out = (freq_factor * (double)base_freq);
...
int start_worker_threads(int num_threads, 
        cfg_t& cfg, uint64_t* pci_vaddr, 
        uint64_t pac_ofw_buf_paddr,
        uint32_t* pac_ofw_buf_vaddr
) {
    int ret;
    ...
    if (cfg.do_dump) {
        ...
        threads_vec.emplace_back(pac_ofw_func, 
                cfg.dump_path, pci_vaddr, cfg.eac_m5,
                pac_ofw_buf_paddr,
                pac_ofw_buf_vaddr);
        //
        workers.push_back(Worker(0));
        // only do this when running pac ofw with m5
        if (cfg.eac_m5) {
...
int start_threads(int num_threads, uint64_t* pci_vaddr, 
        cfg_t cfg, uint64_t pac_ofw_buf_paddr, uint32_t* pac_ofw_buf_vaddr) {
    ...
    cout << cfg.do_dump << " " << cfg.eac_m5 << endl;
    //no_migration = cfg.do_dump;
    //no_algo = cfg.do_dump;
    no_algo = cfg.is_traffic_rate > 0; 
    // test pac with scheme (pac + not_m5)
    if (cfg.do_dump && !cfg.eac_m5) {
        LOG_INFO("dump dmesg in eac_func ...\n");
        ret = start_worker_threads(num_threads, cfg, pci_vaddr, 
                pac_ofw_buf_paddr, pac_ofw_buf_vaddr);
        no_algo = true;
    ...
    producer_func(0, pci_vaddr, sys_counters_curr, cfg, no_algo, no_migration);
