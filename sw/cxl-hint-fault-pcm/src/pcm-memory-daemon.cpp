...
bool anyPmem(const volatile ServerUncoreMemoryMetrics & metrics)
{
...
void printSocketChannelBW(uint32 no_columns, uint32 skt, uint32 num_imc_channels, const std::vector<ServerUncoreCounterState>& uncState1, const std::vector<ServerUncoreCounterState>& uncState2, uint64 elapsedTime, int rankA, int rankB)
{
    for (uint32 channel = 0; channel < num_imc_channels; ++channel) {
        if(rankA >= 0) {
          for (uint32 i=skt; i<(skt+no_columns); ++i) {
              cout << "|-- Mem Ch " << setw(2) << channel << " R " << setw(1) << rankA << ": Reads (MB/s): " << setw(8) << (double) (getMCCounter(channel,ServerUncorePMUs::EventPosition::READ_RANK_A,uncState1[i],uncState2[i]) * 64 / 1000000.0 / (elapsedTime/1000.0)) << " --|";
          }
          cout << "\n";
          for (uint32 i=skt; i<(skt+no_columns); ++i) {
              cout << "|--                Writes(MB/s): " << setw(8) << (double) (getMCCounter(channel,ServerUncorePMUs::EventPosition::WRITE_RANK_A,uncState1[i],uncState2[i]) * 64 / 1000000.0 / (elapsedTime/1000.0)) << " --|";
          }
          cout << "\n";
        }
        if(rankB >= 0) {
          for (uint32 i=skt; i<(skt+no_columns); ++i) {
              cout << "|-- Mem Ch " << setw(2) << channel << " R " << setw(1) << rankB << ": Reads (MB/s): " << setw(8) << (double) (getMCCounter(channel,ServerUncorePMUs::EventPosition::READ_RANK_B,uncState1[i],uncState2[i]) * 64 / 1000000.0 / (elapsedTime/1000.0)) << " --|";
          }
          cout << "\n";
          for (uint32 i=skt; i<(skt+no_columns); ++i) {
              cout << "|--                Writes(MB/s): " << setw(8) << (double) (getMCCounter(channel,ServerUncorePMUs::EventPosition::WRITE_RANK_B,uncState1[i],uncState2[i]) * 64 / 1000000.0 / (elapsedTime/1000.0)) << " --|";
          }
          cout << "\n";
        }
    }
    ...
    const std::vector<ServerUncoreCounterState>& uncState2, const uint64 elapsedTime, const int rankA, const int rankB)
{
    ...
    for (uint32 skt = 0 ; skt < numSockets; ++skt) {
        for (uint32 channel = 0 ; channel < num_imc_channels ; ++channel) {
            if(rankA >= 0) {
                cout << (double) (getMCCounter(channel,ServerUncorePMUs::EventPosition::READ_RANK_A,uncState1[skt],uncState2[skt]) * 64 / 1000000.0 / (elapsedTime/1000.0))
                << "," << (double) (getMCCounter(channel,ServerUncorePMUs::EventPosition::WRITE_RANK_A,uncState1[skt],uncState2[skt]) * 64 / 1000000.0 / (elapsedTime/1000.0)) << ",";
            }
            if(rankB >= 0) {
                cout << (double) (getMCCounter(channel,ServerUncorePMUs::EventPosition::READ_RANK_B,uncState1[skt],uncState2[skt]) * 64 / 1000000.0 / (elapsedTime/1000.0))
                << "," << (double) (getMCCounter(channel,ServerUncorePMUs::EventPosition::WRITE_RANK_B,uncState1[skt],uncState2[skt]) * 64 / 1000000.0 / (elapsedTime/1000.0)) << ",";
            }
        }
    }
    cout << endl;
...
double AD_BW(const memdata_t *md, const uint32 skt)
{
    const auto totalPMM = md->iMC_PMM_Rd_socket[skt] + md->iMC_PMM_Wr_socket[skt];
    return (max)(totalPMM - md->MemoryMode_Miss_socket[skt], double(0.0));
}

double PMM_MM_Ratio(const memdata_t *md, const uint32 skt)
{
...
void display_bandwidth(PCM *m, memdata_t *md, const uint32 no_columns, const bool show_channel_output, const bool print_update, const double CXL_Read_BW)
{
    double sysReadDRAM = 0.0, sysWriteDRAM = 0.0, sysReadPMM = 0.0, sysWritePMM = 0.0;
    ...
    while (skt < numSockets)
    {
        auto printHBM = [&]()
        {
                ...
                if (show_channel_output)
                {
                    double iMC_Rd, iMC_Wr, EDC_Rd, EDC_Wr;
...
constexpr double CXLBWWrScalingFactor = 0.5;

void display_bandwidth_csv(PCM *m, memdata_t *md, uint64 /*elapsedTime*/, const bool show_channel_output, const CsvOutputType outputType, const double CXL_Read_BW)
{
    ...
    memdata_t *md)
{
    ...
    {
        return (double)(nEvents * 64 / 1000000.0 / (elapsedTime / 1000.0));
    };

    auto toRate = [&elapsedTime](const uint64 nEvents)
    {
        return (double)(nEvents / (elapsedTime / 1000.0));
    };
    ...
    for(uint32 skt = 0; skt < m->getNumSockets(); ++skt)
    {
        ...
        if (m->HBMmemoryTrafficMetricsAvailable())
        {
            const double scalingFactor = ((double)m->getHBMCASTransferSize()) / double(64.);
...

        {
            for (uint32 channel = 0; channel < max_imc_channels; ++channel)
            {
                ...
                if (metrics == Pmem)
                {
                    ...
                    md->M2M_NM_read_hit_rate[skt][(channel < numChannels1) ? 0 : 1] += (double)reads;
                }
        ...
        if (metrics == Pmem)
        {
            for (uint32 c = 0; c < max_imc_controllers; ++c)
            {
                ...
                {
                    md->M2M_NM_read_hit_rate[skt][c] = ((double)getM2MCounter(c, ServerUncorePMUs::EventPosition::NM_HIT, uncState1[skt], uncState2[skt])) / md->M2M_NM_read_hit_rate[skt][c];
                }
            }
        }
        if (md->BHS_NM)
    ...
    md->extra.CXL_Read_BW = CXL_Read_BW;
    md->extra.Version++;
...
int mainThrows(int argc, char * argv[])
{
    ...
    double delay = DELAY;
    bool csv = true, csvheader = true, show_channel_output = true, print_update = false;
    ...
    mainLoop([&]()
    {
        ...
        calculate_bandwidth(m,BeforeState,AfterState,AfterTime-BeforeTime,csv,csvheader, no_columns, metrics,
                show_channel_output, print_update, SPR_CHA_CXL_Event_Count, md);
