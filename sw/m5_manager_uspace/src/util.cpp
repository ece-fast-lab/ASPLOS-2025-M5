...
int parse_arg(int argc, char** argv, cfg_t& cfg) {
    ...
    char opt;
    int ret = 0;
    while ((opt = getopt(argc, argv, "t:l:c:s:d:R:x:f:TLCprmnwAh")) != -1) {
        LOG_DEBUG("opt: %c\n", opt);
        switch (opt) {
            case 't':
            ...
            case 'h':
                ...
                printf( 
                        ...
                        "   -R  traffic based query, changing the default rate [see include for default]\n"\
                        " ----------------------------- help ----- <w/o arg>\n"\
                        ...
                        "   -A  no algo. static rate without toggling\n"\
