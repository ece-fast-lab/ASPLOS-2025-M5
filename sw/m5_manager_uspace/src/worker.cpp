...
int start_threads(int num_threads, uint64_t* pci_vaddr, cfg_t cfg) {
    int ret;
    bool no_algo = false;
    ...
    //      output counter without migration. no worker, yes sampler  
        ...
        LOG_INFO("Static rate control \n");
        no_algo = true;
        do_migration = !cfg.no_mig;
    ...
    producer_func(0, pci_vaddr, sys_counters_curr, cfg, no_algo, do_migration);
