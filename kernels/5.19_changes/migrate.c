			   ...
			   int node)
{
    ...
    list_for_each_entry_safe(page_1, page_2, &migratepages, lru) {
        pr_warn("anb X src %lx @ nid %d, dst @ nid %d, pid %d\n",
            page_to_pfn(page_1), info.source_nid, info.target_nid, info.pid
        );
    }
    nr_remaining = 1;
    nr_succeeded = 0;
