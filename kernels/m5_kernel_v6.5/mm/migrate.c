...
static struct folio *alloc_misplaced_dst_folio(struct folio *src,
					   unsigned long data)
{
	struct hintfault_info *info = (struct hintfault_info *)data;
	int nid = info->target_nid;
 ...
 */
int migrate_misplaced_page_real(struct page *page, struct vm_area_struct *vma,
			   int node)
{
	...
	if (page_mapcount(page) != 1 && page_is_file_lru(page) &&
        ...
        //pr_info("migrate_misplaced_page_real out1\n");
		goto out;
    }
	...
	if (page_is_file_lru(page) && PageDirty(page)) {
        //pr_info("migrate_misplaced_page_real out2\n");
		goto out;
    }

	isolated = numamigrate_isolate_page(pgdat, page);
	if (!isolated) {
        //pr_info("migrate_misplaced_page_real out3\n");
		goto out;
    }
    if (vma->vm_mm == NULL) {
        pr_warn("vm_mm is NULL");
        goto out;
        //BUG(1);
    }
    if (vma->vm_mm->owner == NULL) {
        pr_warn("owner is NULL");
        goto out;
        //BUG(1);
    }

	cxl_do_timing(migrate_isolate_time);

	list_add(&page->lru, &migratepages);

	info.target_nid = node;
	info.source_nid = page_to_nid(page);
	info.pid = vma->vm_mm->owner->pid;

	nr_remaining = migrate_pages(&migratepages, alloc_misplaced_dst_folio,
				     NULL, (unsigned long) &info, MIGRATE_ASYNC,
				     MR_NUMA_MISPLACED, &nr_succeeded);
	cxl_do_timing(migrate_return_time);
	if (nr_remaining) {
	...
	cxl_do_counting(migrate_succ_cnt);
    if (isolated == 0) pr_info("migrate_misplaced_page_real isolated=0\n");
	return isolated;

out:
	put_page(page);
	cxl_do_counting(migrate_fail_cnt);
	return 0;
}
			   ...
			   int node)
{
    ...
    // BEFORE, remove log, to check migration overhead again, without allowing migration
    list_for_each_entry_safe(folio, folio2, &migratepages, lru) {
        pr_warn("hintfault src %lx @ nid %d, dst @ nid %d, pid %d\n",
...
#define TRACER_REGION_SIZE (16UL * 1024 * 1024) // 16 MB
...
static unsigned long allocate_tracer_region(int nid)
{
	...
	pr_warn("tracer alloc %ld pages at %lx\n", page_cnt, pa);
...
static void free_tracer_region(unsigned long pa)
{
	...
	pr_warn("tracer free %ld pages at %lx\n", page_cnt, pa);

	free_contig_range(PHYS_PFN(pa), page_cnt);
}

static unsigned long tracer_pa = 0;

static ssize_t tracer_pa_write(struct file *file,
		const char __user *buffer, size_t count, loff_t *ppos)
{
	if (!tracer_pa)
		return -EINVAL;

	free_tracer_region(tracer_pa);
	tracer_pa = 0;

	return count;
}

static int tracer_pa_show(struct seq_file *m, void *v)
{
	if (!tracer_pa)
		tracer_pa = allocate_tracer_region(first_online_node);

	seq_printf(m, "%lx\n", tracer_pa);
	return 0;
}

static int tracer_pa_open(struct inode *inode, struct file *file)
{
	return single_open(file, tracer_pa_show, NULL);
}

static const struct proc_ops tracer_pa_ops = {
	.proc_open	= tracer_pa_open,
	.proc_read	= seq_read,
	.proc_lseek	= seq_lseek,
	.proc_release	= single_release,
	.proc_write	= tracer_pa_write,
};

void register_tracer_pa_proc(void)
{
	proc_create("tracer_pa", 0644, NULL, &tracer_pa_ops);
}
